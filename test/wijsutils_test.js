( function( $ ) {
    ...
    */
    ...
    test( 'wiElementsByClass', 8, function() {
        ...
        strictEqual( 2, wiUtils.wiElementsByClass( 'bar' ).length, 'just a class' );

        // 2nd test: just a class with trailing dot
        strictEqual( 2, wiUtils.wiElementsByClass( '.bar' ).length, 'just a class with trailing dot' );

        // 3rd test: class and node
        strictEqual( 2, wiUtils.wiElementsByClass( 'bar', node ).length, 'class and node' );

        // 4th test: class with trailing dot and node
        strictEqual( 2, wiUtils.wiElementsByClass( '.bar', node ).length, 'class with trailing dot and node' );

        // 5th test: class without tailing dot, node and tag
        strictEqual( 2, wiUtils.wiElementsByClass( 'bar', node, 'div' ).length, 'class without tailing dot, node and tag' );

        // 6th test: class with trailing dot, node and tag
        strictEqual( 2, wiUtils.wiElementsByClass( '.bar', node, 'div' ).length, 'class with trailing dot, node and tag' );

        // 7th test: class without trailing dot, without node (null) and with tag
        strictEqual( 2, wiUtils.wiElementsByClass( 'bar', null, 'div' ).length, 'class without trailing dot, without node and with tag' );

        // 8th test: class with trailing dot, without node and with tag
        strictEqual( 2, wiUtils.wiElementsByClass( '.bar', null, 'div' ).length, 'class with trailing dot, without node and with tag' );
    } );
}( jQuery ) );
